...
const StudentGrades = ({ subjects = [] }) => {
  ...
  // Group subjects by School Year and Term using the pivot data
  const grouped = subjects.reduce((acc, subject) => {
    const pivot = subject.pivot || {};
    // Use the exact database column names: school_year and term
    const key = `${pivot.school_year || 'Unknown Year'} - ${pivot.term || 'Unknown Term'}`;
  ...
  const calculateGWA = (semSubjects) => {
    ...
    semSubjects.forEach((sub) => {
      const grade = parseFloat(sub.pivot?.final_grade);
    ...
    return totalUnits > 0 ? (totalWeightedGrades / totalUnits).toFixed(2) : 'N/A';
  };

  return (
    <div className="space-y-6">
      {Object.entries(grouped).map(([sem, semSubjects]) => (
        <div key={sem} className="border border-gray-100 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="bg-gray-50 px-6 py-4 font-bold text-gray-700 flex justify-between items-center border-b border-gray-100">
            ...
            <span className="text-sm text-gray-500">
               GWA: <span className="font-extrabold text-gray-800 ml-1">{calculateGWA(semSubjects)}</span>
            </span>
          </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="bg-white">
              {semSubjects.map((subject, index) => {
                ...
                const statusBadge = pivot.status === 'Passed' ? 'bg-green-100 text-green-700' : 
                                    pivot.status === 'Failed' ? 'bg-red-100 text-red-700' : 
                                    'bg-gray-100 text-gray-600';

                return (
                  <tr key={index} className="border-b border-gray-50 last:border-none hover:bg-gray-50/50 transition">
                    <td className="px-6 py-4 font-semibold text-gray-700">{subject.code}</td>
                    <td className="px-6 py-4 text-gray-600">{subject.title}</td>
                    <td className="px-6 py-4 text-center text-gray-600">{subject.units}</td>
                    <td className="px-6 py-4 text-center font-bold text-gray-900">{pivot.final_grade || '—'}</td>
                    <td className="px-6 py-4 text-center">
                      <span className={`px-2.5 py-1 text-xs font-bold rounded-md ${statusBadge}`}>
                        {pivot.status || 'Enrolled'}
                      </span>
                    </td>
                  </tr>
                );
